...
function loadbang() {
    loaded = true;
    post("euclidean rhythm generator m4l 1.03 - Robin Price 2011\n If you like and use it why not pay for it?\n");
    ...
    for (i = 0; i < maxparts; i++) {
        ...
        step_boxes[i].message("set", steps[i]);
...
function draw() {
    lcd1.message("clear");
    var rowheight = lcd_height / numparts;
    for (y = numparts - 1; y >= 0; y--) {
        var boxwidth = lcd_width / (2 * steps[y]);
        var arcwidth = 360 / steps[y];
        var arcradius = lcd_height * (y + 1) / (2 * numparts);
        var r = riddims[y];
        for (x = 0; x < steps[y]; x++) {
            if (r[x] == 1) {
                lcd1.message("frgb", 0, 0, 0);
                lcd1.message("pensize", 1, 1);
                lcd1.message("paintarc", lcd_width/4 - arcradius, lcd_width/4 - arcradius, lcd_width/4 + arcradius, lcd_width/4 + arcradius, x * arcwidth, arcwidth + 2);
                lcd1.message("paintrect", boxwidth*x + lcd_width/2, lcd_height - rowheight*(y+1), boxwidth*(x+1) + lcd_width/2, lcd_height - rowheight*y);
            } else if (r[x] == 0.5) {
    ...
    for (y = 0; y < numparts; y++) {
        ...
        for (x = 0; x < steps[y]; x++) {
            lcd1.message("frgb", 255, 255, 255);
            lcd1.message("linesegment", boxwidth*x + lcd_width/2, lcd_height - rowheight * y - 1, boxwidth * x + lcd_width/2, lcd_height - rowheight * (y+1));
        }
    }
}
...
function settime(time) {
    ...
    lcd2.message("linesegment", ((((time + 0.001) * lcd_width)/2) % (lcd_width / 2)) + lcd_width/2, 0, (((time + 0.001) * lcd_width)/2) % (lcd_width / 2) + lcd_width/2, lcd_height);
    lcd2.message("linesegment", lcd_width/4, lcd_height/2, lcd_width/4 + (lcd_width/4) * Math.sin(-time*Math.PI*2 - Math.PI), lcd_width/4 + (lcd_width/4) * Math.cos(-time*Math.PI*2 - Math.PI));
}